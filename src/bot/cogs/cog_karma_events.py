...
class KarmaBotReactionCog(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        self.bot.logger.debug(f'Add reaction, message sender id: {sender_id}')
        if self.bot.reaction_is_upvote(payload.emoji):
            await self.bot.store.upvote_user(payload.guild_id, payload.user_id)
        elif self.bot.reaction_is_downvote(payload.emoji):
            await self.bot.store.downvote_user(payload.guild_id, payload.user_id)
